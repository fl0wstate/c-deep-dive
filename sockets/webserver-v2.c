...
// a function that will be used to rbroadcast the message to all the clients
                             ...
                             int client_fd, uint32_t *n_pollfds)
{
  for (uint32_t i = 0; i < *n_pollfds; i++)
  {
    ...
    if ((*pollfds)[i].fd != client_fd && (*pollfds)[i].fd != 1)
    {
      ...
      {
        LOG(ERROR,
            "Failed to send message to client_fd [%d]. Closing connection.",
            (*pollfds)[i].fd);
...
void send_pong(int client_fd, const unsigned char *payload, size_t payload_len)
{
    ...
    else
    {
      ...
      for (uint32_t i = 0; i < n_pollfds; i++)
      {
        ...
        if (pollfds[i].revents & POLLIN)
        {
          ...
          {
            LOG(DEBUG, "Reading from the client [%d]", pollfds[i].fd);
            // read from the client
            if (receive_text_frame(pollfds[i].fd, &pollfds, &n_pollfds,
                                   message) < 0)
            {
