...
void disconnectCallback(const redisAsyncContext *ac, int status) {
  // update the status
  if (status != REDIS_OK) {
    LOG(ERROR, "Error while disconnecting: %s", ac->errstr);
    return;
  }
...
void generic_callback(redisAsyncContext *ac, void *r, void *privatedata) {
  redisReply *reply = r;
  struct connection connect_st;

  if (reply == NULL || ac->err) {
    fprintf(stderr, "Error: %s", ac->errstr);
    return;
  }
  LOG(DEBUG, "%s", (char *)privatedata);
  ...
  case REDIS_REPLY_STRING:
    ...
    connect_st.connection = atoi(reply->str);
    break;
...
int accept_connections(int server_fd, struct pollfd **pollfds,
  ...
  if (*n_pollfds > 2) {
    ...
    const char *set[] = {"SET", "user:connected", data};
    ...
    LOG(DEBUG, "Connection -> %d", connected.connection);

    // message to be shared over all the clients
...
int main(void) {
  ...
  // ********************************************************************************
  // setup the redis client connection
  redisPollAttach(ac);
